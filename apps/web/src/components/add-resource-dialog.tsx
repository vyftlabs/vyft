...
export function AddResourceDialog({
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange} modal={!container}>
      <DialogContent className="sm:max-w-lg flex flex-col p-0 gap-0 overflow-hidden" showCloseButton={dismissible} container={container}>
        <Command className="rounded-none border-0 flex flex-col flex-1 min-h-0">
          ...
          <CommandList className="h-80 max-h-none pt-1">
            ...
            <CommandGroup heading="Sources">
              <CommandItem onSelect={() => onSelect("service", { type: "image" })} data-testid="service-source-image-option">
                <ContainerIcon className="text-muted-foreground" />
                Container Image
              </CommandItem>
            </CommandGroup>
            {[...new Set(componentDefinitions.map((d) => d.category))].map((category) => (
              <CommandGroup key={category} heading={category}>
                {componentDefinitions.filter((d) => d.category === category).map((def) => {
                  const Icon = def.icon
                  return (
