...
function MiniNode({ data }: { data: { label: string; status: ServiceStatus; image?: string } }) {
  ...
  return (
    <div className={cn(
      ...
      <span className="text-[8px] font-medium text-foreground/70 truncate">{data.label}</span>
    </div>
  )
}
...
export function MiniCanvas({ services, className }: { services: ProjectServiceSummary[]; className?: string }) {
  const nodes: Node[] = services.map((svc, i) => ({
    id: svc.id,
    position: { x: (i % COLS) * X_GAP, y: Math.floor(i / COLS) * Y_GAP },
    data: { label: svc.name, status: svc.status as ServiceStatus, image: svc.image },
    type: "mini",
    draggable: false,
    selectable: false,
    connectable: false,
...
export function ProjectCard({ name, status, statusMessage, services = [], href, onClick }: ProjectCardProps) {
  ...
  return (
    ...
    >
      {services.length > 0 ? (
        <MiniCanvas services={services} className="border-b border-foreground/[0.04]" />
      ) : (
      ...
      <div className="p-3 flex items-center justify-between gap-2">
        ...
        {status && status !== "healthy" && statusMessage && (
          <span className={cn("text-[10px] font-medium px-1.5 py-0.5 rounded-full shrink-0", statusBadgeClass[status])}>
            {statusMessage}
          </span>
        )}
      </div>
