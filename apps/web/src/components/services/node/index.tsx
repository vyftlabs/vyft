...
export function ServiceIcon({ image, size = "md" }: { image?: string; size?: keyof typeof iconSizes }) {
...
function useRecentlyDeployed(deployedAt?: string): { minutesAgo: number; timeAgo: string } {
  ...
  useEffect(() => {
    ...
    const update = () => setMinutesAgo((Date.now() - new Date(deployedAt).getTime()) / 60_000)
    update()
...
const statusTheme: Partial<Record<ServiceState, { glow: string; ring: string; text: string }>> = {
  pending:     { glow: "bg-blue-400/60",    ring: "ring-blue-400/20 dark:ring-blue-400/25",    text: "text-blue-400/80" },
  degraded:    { glow: "bg-orange-400/60",   ring: "ring-orange-400/20 dark:ring-orange-400/25", text: "text-orange-400/80" },
  failed:      { glow: "bg-rose-400/60",     ring: "ring-rose-400/20 dark:ring-rose-400/25",    text: "text-rose-400/80" },
  unknown:     { glow: "bg-purple-400/60",   ring: "ring-purple-400/20 dark:ring-purple-400/25", text: "text-purple-400/80" },
  terminating: { glow: "bg-zinc-400/40",     ring: "ring-zinc-400/20 dark:ring-zinc-400/25",    text: "text-zinc-400/80" },
}

export function ServiceNodeCard({ label, image, status, deployedAt, onClick, onHover }: ServiceNodeData) {
  const theme = statusTheme[status.state]
  const { minutesAgo, timeAgo } = useRecentlyDeployed(deployedAt)
  const recentlyDeployed = status.state === "running" && minutesAgo < RECENTLY_DEPLOYED_MINUTES
  const recentOpacity = recentlyDeployed ? 1 - minutesAgo / RECENTLY_DEPLOYED_MINUTES : 0
      ...
      >
        <div className="p-3 space-y-1">
          <div className="flex items-center gap-2">
            <div className="shrink-0 text-foreground/80"><ServiceIcon image={image} /></div>
