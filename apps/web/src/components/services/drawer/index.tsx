  ...
  return (
    <div className="flex flex-col gap-5 h-full">
      {(sparklines.length > 0 || latency) && (
        <div className="space-y-2 shrink-0">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onViewAll?.("metrics")} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors">
              View all
            </button>
          </div>
          ...
          {sparklines[1] && (
            <div className={cn("grid gap-2", `grid-cols-${sparklines[1].length}`)}>
              {sparklines[1].map((s) => (
                <Sparkline key={s.dataKey} {...s} />
              ))}
            </div>
      ...
      {(showTimeline || showLogs) && (
        <div className={cn("flex-1 min-h-0 grid gap-4", showTimeline && showLogs ? "grid-cols-2" : "grid-cols-1")}>
          {showTimeline && (
            <div className="min-h-0 flex flex-col">
              <div className="flex items-center justify-between mb-2 shrink-0">
                <p className="text-xs font-medium">Events</p>
                <button onClick={() => onViewAll?.("events")} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors">
                  View all
                </button>
              </div>
          ...
          {showLogs && (
            <div className="min-h-0 flex flex-col">
              <div className="flex items-center justify-between mb-2 shrink-0">
                ...
                <button onClick={() => onViewAll?.("logs")} className="text-[11px] text-muted-foreground hover:text-foreground transition-colors">
                  View all
                </button>
              </div>
              <div className="flex-1 min-h-0 relative">
...
function LogsPreview({ logs }: { logs: LogLine[] }) {
  return (
    <div className="font-mono text-[11px] leading-relaxed space-y-px">
      {logs.map((log, i) => (
        <div key={i} className="flex gap-2 px-1 hover:bg-muted/50 rounded-sm">
          <span className="text-muted-foreground shrink-0">
            {new Date(log.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit", second: "2-digit", hour12: false })}
          </span>
          <span className={cn("shrink-0 w-10 uppercase", logLevelClass[log.level])}>
            {log.level}
          </span>
...
export function Sparkline({ title, data, dataKey, unit, threshold }: SparklineData) {
  ...
  const overlayData = threshold ? data.map((d) => {
  ...
  return (
    <div className="p-3">
      ...
      <p className={cn("text-lg font-semibold font-mono", level && severityTextClass[level])}>
        {fmt(current)}<span className={cn("text-xs ml-0.5", level ? severityTextClass[level] : "text-muted-foreground")}>{unit}</span>
      </p>
      <div className="h-10 mt-1" style={{ minWidth: 0, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
          <AreaChart data={overlayData} margin={{ top: 2, right: 0, bottom: 0, left: 0 }}>
            <defs>
...
export function LatencySparkline({ data, keys, unit = "ms" }: LatencyChartData) {
  ...
  const secondaryValues = keys
    ...
    .map((k) => ({
      label: k.label,
      value: percentile(data.map((d) => (d[k.dataKey] as number) ?? 0), 95),
  ...
  return (
    <div className="p-3">
      ...
      <p className={cn("text-lg font-semibold font-mono", headlineLevel && severityTextClass[headlineLevel])}>
        {fmt(headline)}<span className={cn("text-xs ml-0.5", headlineLevel ? severityTextClass[headlineLevel] : "text-muted-foreground")}>{unit}</span>
      </p>
      ...
      <div className="h-10 mt-1" style={{ minWidth: 0, minHeight: 0 }}>
        <ResponsiveContainer width="100%" height="100%" minWidth={1} minHeight={1}>
          <AreaChart data={data} margin={{ top: 2, right: 0, bottom: 0, left: 0 }}>
            <defs>
              <linearGradient id="spark-latency-fill" x1="0" y1="0" x2="0" y2="1">
              ...
              {keys.map((k) => {
                ...
                return (
                  <linearGradient key={k.dataKey} id={`spark-latency-stroke-${k.dataKey}`} x1="0" y1="0" x2="0" y2="1">
                    <stop offset={`${cPct * 100}%`} stopColor="var(--color-severity-critical)" />
                    <stop offset={`${cPct * 100}%`} stopColor="var(--color-severity-warning)" />
                    <stop offset={`${wPct * 100}%`} stopColor="var(--color-severity-warning)" />
                    <stop offset={`${wPct * 100}%`} stopColor="var(--primary)" />
                  </linearGradient>
                )
              })}
            </defs>
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <div className="rounded-md bg-popover px-2 py-1.5 shadow-md ring-1 ring-foreground/10 space-y-0.5">
                    {keys.map((k) => {
                      const entry = payload.find((p) => p.dataKey === k.dataKey)
                      if (!entry) return null
                      const tt_level = severity(entry.value as number, k.threshold)
                      return (
                        <div key={k.dataKey} className="flex items-center justify-between gap-3 text-xs font-mono">
                          <span className="text-muted-foreground">{k.label}</span>
                          <span className={cn(tt_level && severityTextClass[tt_level])}>
                            {fmt(entry.value as number)}{unit}
                          </span>
                        </div>
                      )
                    })}
            ...
            {[...keys].reverse().map((k) => {
              ...
              return (
                <Area
                  ...
                  type="monotone"
                  dataKey={k.dataKey}
                  stroke={k.threshold ? `url(#spark-latency-stroke-${k.dataKey})` : "var(--primary)"}
  ...
  return (
    <>
      ...
      <motion.div
        ...
        initial={skipEntryAnimation ? false : { opacity: 0, scale: 0.96, x: 16, y: 16, bottom: 0, right: 0, width: "100%", maxWidth: "min(64rem, calc(100vw - 2rem))", height: "80%", borderTopLeftRadius: 8, borderWidth: 1 }}
        animate={expanded
          ? { opacity: 1, scale: 1, x: 0, y: 0, bottom: 0, right: 0, left: 0, top: 0, width: "100%", maxWidth: "100%", height: "100%", borderTopLeftRadius: 0, borderWidth: 0 }
          : { opacity: 1, scale: 1, x: 0, y: 0, bottom: 0, right: 0, width: "100%", maxWidth: "min(64rem, calc(100vw - 2rem))", height: "80%", borderTopLeftRadius: 8, borderWidth: 1 }
        }
        exit={{ opacity: 0, scale: 0.98, x: 10, y: 10 }}
        transition={drawerTransition}
        style={{ minHeight: "24rem", borderColor: "var(--border)", borderStyle: "solid" }}
      >
        {expanded ? (
          expandedContent
        ) : (
          <>
            <div className="flex items-center justify-between px-6 py-4 shrink-0">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                {typeof name === "string" ? <p className="text-lg font-semibold truncate">{name}</p> : name}
              </div>
            ...
            {tabs.length > 0 && (
              <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col flex-1 min-h-0">
                {tabs.length > 1 && (
                  <TabsList variant="line" className="px-6 border-b !w-full !justify-start [&>*]:!flex-none [&>*]:mb-[-1px]">
                    {tabs.map((tab) => (
                ...
                {banner}
                {tabs.map((tab) => (
                  <TabsContent key={tab.id} value={tab.id} className="flex-1 min-h-0 px-6 pt-4 pb-6">
                    {tab.content}
                  </TabsContent>
                ))}
              </Tabs>
