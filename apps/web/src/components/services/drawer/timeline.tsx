...
import { reasonCategory, type KubeEventCategory } from "./kube-events"
...
export interface Deployment {
  ...
  status: "rolling_out" | "succeeded" | "failed" | "rolling_back" | "rolled_back"
  startedAt: string
  completedAt?: string
  actor?: string
...
export type ActionType = "scaled" | "config_updated" | "variable_added" | "variable_removed" | "route_added" | "route_removed" | "volume_attached" | "volume_detached" | "restarted"
...
const deploymentStatusIcon: Record<Deployment["status"], React.ReactNode> = {
  rolling_out: <LoaderIcon className="size-3.5 text-severity-info animate-spin" />,
  succeeded: <CheckIcon className="size-3.5 text-emerald-600 dark:text-emerald-400" />,
  failed: <XIcon className="size-3.5 text-severity-critical-text" />,
  rolling_back: <LoaderIcon className="size-3.5 text-severity-warning-text animate-spin" />,
...
const categoryIcon: Record<KubeEventCategory, (warning: boolean) => React.ReactNode> = {
  container: (w) => w ? <RefreshCwIcon className="size-3 text-severity-warning-text" /> : <CheckIcon className="size-3 text-muted-foreground" />,
  image: (w) => w ? <DownloadIcon className="size-3 text-severity-warning-text" /> : <DownloadIcon className="size-3 text-muted-foreground" />,
  probe: () => <HeartPulseIcon className="size-3 text-severity-warning-text" />,
  scheduling: (w) => w ? <ServerOffIcon className="size-3 text-severity-warning-text" /> : <ServerIcon className="size-3 text-muted-foreground" />,
  scaling: () => <ArrowUpDownIcon className="size-3 text-muted-foreground" />,
  volume: (w) => w ? <HardDriveIcon className="size-3 text-severity-warning-text" /> : <HardDriveIcon className="size-3 text-muted-foreground" />,
  node: (w) => w ? <ServerOffIcon className="size-3 text-severity-critical-text" /> : <ServerIcon className="size-3 text-muted-foreground" />,
  network: (w) => w ? <WifiOffIcon className="size-3 text-severity-warning-text" /> : <NetworkIcon className="size-3 text-muted-foreground" />,
...
function eventIcon(reason: string, type: "Normal" | "Warning"): React.ReactNode {
...
function DeploymentEntry({ deployment }: { deployment: Deployment }) {
  const [open, setOpen] = useState(deployment.status === "failed" || deployment.status === "rolling_out" || deployment.status === "rolling_back")
  ...
  return (
    <div>
      ...
      >
        <span className="mt-0.5 shrink-0">{deploymentStatusIcon[deployment.status]}</span>
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            <div className="flex items-center gap-1.5 min-w-0">
              <span className={cn("text-xs font-medium", deploymentStatusText[deployment.status])}>
                {deploymentStatusLabel[deployment.status]}
              </span>
              <span className="text-xs font-mono text-foreground truncate">{imageTag(deployment.image)}</span>
            </div>
            <div className="flex items-center gap-2 shrink-0">
              {duration && (
                <span className="text-[10px] text-muted-foreground font-mono">{duration}</span>
              )}
              <span className="text-[10px] text-muted-foreground">{timeAgo(deployment.startedAt)}</span>
              {hasEvents && (
          ...
          <div className="flex items-center gap-1.5 mt-0.5">
            {deployment.commit && (
              <span className="text-[10px] text-muted-foreground font-mono">
                {deployment.commit.url ? (
                  <a href={deployment.commit.url} target="_blank" rel="noopener noreferrer" className="hover:underline underline-offset-2 hover:text-foreground transition-colors" onClick={(e) => e.stopPropagation()}>
            ...
            {deployment.actor && (
              <span className="text-[10px] text-muted-foreground">{deployment.actor}</span>
            )}
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="text-[10px] text-muted-foreground">{timeAgo(action.timestamp)}</span>
              {hasEvents && (
                <ChevronRightIcon className={cn(
                  "size-3 text-muted-foreground transition-transform",
                  open && "rotate-90",
          ...
          {action.actor && (
            <p className="text-[10px] text-muted-foreground mt-0.5">{action.actor}</p>
          )}
        </div>
      </button>
...
function KubeEventRow({ event }: { event: KubeEvent }) {
  return (
    <div className="py-1 flex items-start gap-2">
      <span className="mt-0.5 shrink-0">{eventIcon(event.reason, event.type)}</span>
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2">
          <p className={cn(
            "text-[11px] font-medium",
            event.type === "Warning" ? "text-severity-warning-text" : "text-muted-foreground",
          )}>
            {event.reason}
          </p>
          <span className="text-[10px] text-muted-foreground shrink-0">{timeAgo(event.timestamp)}</span>
        </div>
        <p className="text-[10px] text-muted-foreground leading-snug">{event.message}</p>
      </div>
    </div>
  )
}
    ...
    <div className="space-y-0.5">
      {entries.map((entry) => {
        if (entry.kind === "deployment") {
          return <DeploymentEntry key={entry.deployment.id} deployment={entry.deployment} />
        }
