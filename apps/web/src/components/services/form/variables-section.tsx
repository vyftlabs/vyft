...
export function VariablesSection({ control, projectId }: { control: Control<ServiceFormValues>; projectId: string }) {
  ...
  return (
    <Variables
      variables={fields.map((f) => ({ key: f.key, value: f.value, secret: f.secret }))}
      onDelete={(key) => {
    ...
    >
      ...
      <AddVariableDialog
        ...
        onAddLocal={(v) => append({ key: v.key, value: v.value, secret: v.secret, sourceVariableId: v.sourceVariableId })}
      />
    </Variables>
  )
}
