...
function findResourceContainingRoute(routeId: string): Resource | undefined {
  return store.read("resources").find((r) =>
    r.service?.routes?.some((rt) => rt.id === routeId),
  )
}
...
async function listRoutes(projectId: string, serviceId: string): Promise<Route[]> {
  await delay()
...
async function createRoute(projectId: string, serviceId: string, body: RouteCreate): Promise<Route> {
  await delay()
  const r = findResourceByService(serviceId)
  if (!r || r.projectId !== projectId || !r.service) throw notFound("Service not found")
  const route: Route = {
    id: uuid(),
    serviceId,
    domain: body.domain,
...
async function updateRoute(projectId: string, id: string, body: RouteUpdate): Promise<Route> {
  await delay()
  const r = findResourceContainingRoute(id)
  if (!r || r.projectId !== projectId || !r.service) throw notFound("Route not found")
  const list = r.service.routes ?? []
  const idx = list.findIndex((rt) => rt.id === id)
  if (idx === -1) throw notFound("Route not found")
  const updated: Route = { ...list[idx]!, ...body, updatedAt: now() }
  const next = [...list]
  next[idx] = updated
...
export const create = mutationOptions({
  mutationFn: ({ projectId, serviceId, body }: { projectId: string; serviceId: string; body: RouteCreate }) =>
...
export const update = mutationOptions({
  mutationFn: ({ projectId, id, body }: { projectId: string; id: string; body: RouteUpdate }) =>
