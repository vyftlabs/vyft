...
async function getResource(projectId: string, id: string): Promise<Resource> {
  await delay()
  const r = store.read("resources").find((r) => r.id === id && r.projectId === projectId)
  if (!r) throw notFound("Resource not found")
  return r
}

async function createResource(projectId: string, body: ResourceCreate): Promise<Resource> {
  await delay()
  const existing = store.read("resources").find(
    (r) => r.projectId === projectId && r.name === body.name,
  )
...
async function updateResource(projectId: string, id: string, body: ResourceUpdate): Promise<Resource> {
  await delay()
  ...
  const updated: Resource = {
    ...
    service: r.service && {
      ...
      app: r.service.app && {
        ...
        port: body.port !== undefined ? (body.port ?? null) : r.service.app.port,
        command: body.command !== undefined ? (body.command ?? null) : r.service.app.command,
...
async function updateResourcePosition(projectId: string, id: string, body: ResourcePosition): Promise<Resource> {
  await delay(50)
  const list = store.read("resources")
  const idx = list.findIndex((r) => r.id === id && r.projectId === projectId)
  if (idx === -1) throw notFound("Resource not found")
...
async function deleteResource(projectId: string, id: string): Promise<void> {
  ...
  const r = store.read("resources").find((r) => r.id === id && r.projectId === projectId)
  if (!r) throw notFound("Resource not found")
  store.write("resources", store.read("resources").filter((r) => r.id !== id))
  store.write("variables", store.read("variables").filter((v) => v.resourceId !== id))
}
...
export const create = mutationOptions({
  mutationFn: ({ projectId, body }: { projectId: string; body: ResourceCreate }) =>
...
export const update = mutationOptions({
  mutationFn: ({ projectId, id, body }: { projectId: string; id: string; body: ResourceUpdate }) =>
...
export const updatePosition = mutationOptions({
  mutationFn: ({ projectId, id, body }: { projectId: string; id: string; body: ResourcePosition }) =>
