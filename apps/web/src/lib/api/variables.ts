...
function withSourceMeta(v: Variable): Variable {
  ...
  const source = store.read("variables").find((s) => s.id === v.sourceVariableId)
  if (!source) return v
...
async function listVariables(projectId: string, opts: { resourceId?: string } = {}): Promise<Variable[]> {
  await delay()
  const all = store.read("variables").filter((v) => v.projectId === projectId)
  if (opts.resourceId) {
    const scoped = all.filter((v) => v.scope === "resource" && v.resourceId === opts.resourceId)
    return scoped.map((v) => withRedaction(withSourceMeta(v)))
  }
  return all.filter((v) => v.scope === "shared").map((v) => {
    const usedBy = all
      .filter((other) => other.sourceVariableId === v.id && other.resourceId)
      .map((other) => {
        const r = store.read("resources").find((r) => r.id === other.resourceId)
...
async function listReferences(projectId: string): Promise<VariableReference[]> {
  ...
  return all
    ...
    .map((v) => {
      ...
      if (!source?.resourceId || source.resourceId === v.resourceId) return null
      return { sourceResourceId: source.resourceId, targetResourceId: v.resourceId! }
    })
    .filter(Boolean) as VariableReference[]
}

async function listSuggestions(projectId: string, opts: { excludeResourceId?: string } = {}): Promise<VariableSuggestions> {
  await delay()
  const allVars = store.read("variables").filter((v) => v.projectId === projectId)
  const allResources = store.read("resources").filter((r) => r.projectId === projectId)
  ...
  const service: SuggestionService[] = allVars
    .filter((v) => v.scope === "resource" && !v.sourceVariableId && v.resourceId !== opts.excludeResourceId)
    .map((v) => {
...
async function getVariable(projectId: string, id: string): Promise<Variable> {
  await delay()
  const v = store.read("variables").find((v) => v.id === id && v.projectId === projectId)
  if (!v) throw notFound("Variable not found")
  return withRedaction(withSourceMeta(v))
}

async function createVariable(projectId: string, body: VariableCreate): Promise<Variable> {
  await delay()
  const scope: "shared" | "resource" = body.resourceId ? "resource" : "shared"
  const conflictKey = scope === "shared"
    ? store.read("variables").find(
        (v) => v.projectId === projectId && v.scope === "shared" && v.key === body.key,
      )
    : store.read("variables").find(
        (v) => v.scope === "resource" && v.resourceId === body.resourceId && v.key === body.key,
      )
...
async function updateVariable(projectId: string, id: string, body: VariableUpdate): Promise<Variable> {
  await delay()
  ...
  if (body.key && body.key !== v.key) {
    const conflictKey = v.scope === "shared"
      ? list.find((o) => o.projectId === projectId && o.scope === "shared" && o.key === body.key)
      : list.find((o) => o.scope === "resource" && o.resourceId === v.resourceId && o.key === body.key)
  ...
  const updated: Variable = {
    ...
    sourceVariableId: body.sourceVariableId !== undefined ? (body.sourceVariableId ?? null) : v.sourceVariableId,
    updatedAt: now(),
  }
  const next = [...list]
  next[idx] = updated
...
async function deleteVariable(projectId: string, id: string): Promise<void> {
  ...
  if (!v) throw notFound("Variable not found")
  if (v.scope === "shared") {
    const refs = store.read("variables").filter((o) => o.sourceVariableId === id)
    if (refs.length > 0) {
      const names = refs
        .map((r) => store.read("resources").find((res) => res.id === r.resourceId)?.name)
        .filter(Boolean)
      throw badRequest(`Cannot delete: referenced by ${names.join(", ")}`)
    }
  }
  store.write("variables", store.read("variables").filter((v) => v.id !== id))
}
...
export const suggestions = (projectId: string, opts: { excludeResourceId?: string } = {}) =>
  queryOptions({
    queryKey: [...ROOT, projectId, "suggestions", opts.excludeResourceId ?? null],
...
export const create = mutationOptions({
  mutationFn: ({ projectId, body }: { projectId: string; body: VariableCreate }) =>
...
export const update = mutationOptions({
  mutationFn: ({ projectId, id, body }: { projectId: string; id: string; body: VariableUpdate }) =>
