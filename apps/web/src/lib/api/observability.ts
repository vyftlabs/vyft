...
const EVENT_TEMPLATES = [
  ...
  },
  {
    ...
    reason: "Unhealthy",
    message: "Liveness probe failed: HTTP 503",
  },
  {
    type: "Warning" as const,
    reason: "BackOff",
...
async function listEvents(
  ...
  return Array.from({ length: count }, (_, i) => {
    const ti = Math.floor(rand() * EVENT_TEMPLATES.length);
    const template = EVENT_TEMPLATES[ti];
    if (template === undefined) {
      throw new Error("event template index");
    }
    return {
...
async function listLogs(
  ...
  return Array.from({ length: count }, (_, i) => {
    const ti = Math.floor(rand() * LOG_TEMPLATES.length);
    const template = LOG_TEMPLATES[ti];
    if (template === undefined) {
      throw new Error("log template index");
    }
    return {
