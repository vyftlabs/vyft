...
async function updateProject(id: string, body: ProjectUpdate): Promise<Project> {
  await delay()
  ...
  const updated: Project = {
    ...list[idx]!,
    ...body,
    description: body.description !== undefined ? (body.description ?? null) : list[idx]!.description,
    updatedAt: now(),
  }
  const next = [...list]
  next[idx] = updated
...
async function deleteProject(id: string): Promise<void> {
  ...
  store.write("projects", store.read("projects").filter((p) => p.id !== id))
  store.write("resources", store.read("resources").filter((r) => r.projectId !== id))
  store.write("variables", store.read("variables").filter((v) => v.projectId !== id))
  store.write("deployments", store.read("deployments").filter((d) => d.projectId !== id))
}
