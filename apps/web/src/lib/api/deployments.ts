...
import type { Deployment, DeploymentChecksum, DeploymentLatest } from "@vyft/spec"
...
function computeChecksum(projectId: string): string {
  ...
  if (!project) return ""
  const resources = store.read("resources").filter((r) => r.projectId === projectId)
  const variables = store.read("variables").filter((v) => v.projectId === projectId)
...
async function getLatest(projectId: string): Promise<DeploymentLatest> {
  ...
  return { checksum: latest.checksum, status: latest.status, createdAt: latest.createdAt }
}
...
async function createDeployment(projectId: string): Promise<Deployment> {
  ...
  setTimeout(() => updateDeployment(deployment.id, { status: "applying" }), 1000)
  setTimeout(() => updateDeployment(deployment.id, {
    status: "applied",
...
export const create = mutationOptions({
  mutationFn: ({ projectId }: { projectId: string }) => createDeployment(projectId),
