...
async function listVolumes(projectId: string, serviceId: string): Promise<Volume[]> {
  await delay()
...
async function createVolume(projectId: string, serviceId: string, body: VolumeCreate): Promise<Volume> {
  await delay()
  const r = findResourceByService(serviceId)
  ...
  writeResource({
    ...r,
    service: { ...r.service, volumeMounts: [...(r.service.volumeMounts ?? []), mount] },
  })
  return volume
}
...
async function deleteVolume(projectId: string, id: string): Promise<void> {
  ...
  if (!r || r.projectId !== projectId || !r.service) throw notFound("Volume not found")
  const next = (r.service.volumeMounts ?? []).filter((vm) => vm.volume.id !== id)
...
export const create = mutationOptions({
  mutationFn: ({ projectId, serviceId, body }: { projectId: string; serviceId: string; body: VolumeCreate }) =>
