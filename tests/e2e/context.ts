...
import type { K8sClient } from "../../src/runtimes/k8s/client.ts";
...
export class DockerTestContext extends TestContext {
  ...
  async serviceReady(id: string, timeoutMs = 60_000): Promise<void> {
    ...
    while (Date.now() < deadline) {
      try {
        ...
        // No health check configured — running is the best we can do
        ...
        // Container may not exist yet (e.g. Swarm scheduling delay) — keep polling
        lastError = err;
      }
  ...
  async cronjobReady(id: string, timeoutMs = 60_000): Promise<void> {
    // Cronjob scheduler container must be running (supercronic stays alive)
...
export class K8sTestContext extends TestContext {
  ...
  async serviceReady(id: string, timeoutMs = 60_000): Promise<void> {
    const deadline = Date.now() + timeoutMs;
    while (Date.now() < deadline) {
      const pods = await this.k8s.core.listNamespacedPod({
  ...
  async cronjobReady(id: string, timeoutMs = 30_000): Promise<void> {
    ...
    while (Date.now() < deadline) {
      try {
        await this.k8s.batch.readNamespacedCronJob({
          namespace: this.namespace,
          name: id,
        });
        return;
