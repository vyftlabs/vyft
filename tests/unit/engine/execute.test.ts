...
import type { ExecuteEvent } from "../../../src/engine/execute.ts";
...
import { svc, vol } from "./helpers.ts";
...
describe("execute", () => {
  ...
  it("waits for dependencies before starting dependents", async () => {
    ...
    // Volume must finish before service starts
    ok(
  ...
  it("handles recreate (multi-op) sequentially per resource but parallel across resources", async () => {
    ...
    // Each resource's remove happens before its create
  ...
  it("emits events with graph-based parallel execution", async () => {
    ...
    // Both resources should have before/after events
    ...
    // before should come before after for each resource
