...
describe("FileStore", () => {
  ...
  it("delete is idempotent on missing directory", async () => {
    const store = createFileStore(root);
    await store.delete("ctx", "nonexistent"); // should not throw
  });
});
...
describe("WAL", () => {
  ...
  it("load replays committed WAL entries on top of checkpoint", async () => {
    ...
    // Append a committed entry for a new resource
  ...
  it("compact writes state and removes WAL", async () => {
    ...
    // Compact with the replayed state
    ...
    // State should be clean and include both resources
    const loaded = await store.load("ctx", "proj");
  ...
  it("pending WAL entries do not affect loaded resources", async () => {
    ...
    // "pending" entries are just markers, they don't change resource state
    await store.appendLog("ctx", "proj", {
      type: "pending",
      id: "a",
      operation: "updating",
