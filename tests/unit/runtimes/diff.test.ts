...
import type { Secret, Service, Volume } from "../../../src/resource.ts";
import {
  changedFields,
  hasSpecChange,
  NON_SPEC_FIELDS,
} from "../../../src/runtimes/diff.ts";
...
describe("changedFields", () => {
  ...
  it("handles serialization asymmetry for volume mounts", () => {
    const v = vol("data");
    // previous has ID string (from JSON roundtrip)
    const prev = { mounts: [{ volume: "data", path: "/d" }] };
    // config has live Volume object
