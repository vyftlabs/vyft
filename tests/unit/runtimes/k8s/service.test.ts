...
import {
  buildClusterIPManifest,
  buildDeploymentManifest,
} from "../../../../src/runtimes/k8s/service.ts";
...
describe("buildDeploymentManifest", () => {
  ...
  it("includes volume mounts", () => {
    const vol = { kind: "volume" as const, id: "data", config: {} };
    const m = buildDeploymentManifest(
      "api",
      "ns",
      "api",
  ...
  it("includes health probes", () => {
    ...
    const containers = m.spec.template.spec.containers as Record<
      ...
      unknown
    >[];
    const liveness = containers[0]?.livenessProbe as Record<string, unknown>;
    ok(liveness);
