import type { RuntimeName } from "../src/config.ts";
import { deploy } from "../src/engine/index.ts";
import type { Change } from "../src/engine/plan.ts";
import type { Resource, Service } from "../src/resource.ts";
import { changedFields, hasSpecChange } from "../src/runtimes/diff.ts";
import { createDockerRuntime } from "../src/runtimes/docker/index.ts";
import { createK8sRuntime } from "../src/runtimes/k8s/index.ts";
import { createSwarmRuntime } from "../src/runtimes/swarm/index.ts";
import type { ExtendedRuntime, Operation } from "../src/runtimes/types.ts";
...
export class Simulation {
  ...
  constructor(runtimeType?: RuntimeName) {
    if (!runtimeType) {
      ...
      // plan() is a pure function of its Change argument — it never accesses
      // `this` or closure state, so detaching the method reference is safe.
