...
describe("failure injection", () => {
  ...
  it("failOn with once=true fires only once", async () => {
    sim.runtime.failOn(
      (op) => op.action === "create" && op.resource.kind === "volume",
      new Error("disk full"),
      true,
    );
  ...
  it("failOn with once=false fails every matching operation", async () => {
    ...
    sim2.runtime.failOn(
      (op) => op.action === "create" && op.resource.kind === "service",
...
describe("operation log", () => {
  ...
  it("log indices continue across deploys", async () => {
    ...
    ok(secondMin !== undefined, "secondMin should be defined");
    ok(firstMax !== undefined, "firstMax should be defined");
    ok(
      secondMin > firstMax,
  ...
  it("lastOps returns only the most recent deploy's operations", async () => {
    ...
    ok(op, "operation should be defined");
