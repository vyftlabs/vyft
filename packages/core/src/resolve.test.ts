...
import { resolve, resolveEnv } from "@vyft/core";
import type { VariableRef } from "@vyft/primitives";
...
describe("resolve", () => {
  ...
  it("resolves a secret reference", () => {
    const ref: VariableRef = { kind: "variable", id: "db-pass" };
    strictEqual(resolve(ref, secrets), "s3cret");
  });

  it("throws on missing secret", () => {
    const ref: VariableRef = { kind: "variable", id: "missing" };
  ...
  it("resolves interpolation with multiple secrets", () => {
    const a: VariableRef = { kind: "variable", id: "db-pass" };
    const b: VariableRef = { kind: "variable", id: "api-key" };
